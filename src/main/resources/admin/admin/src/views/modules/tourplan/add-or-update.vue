<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="目的地" prop="mudedi">
					<el-input v-model="ruleForm.mudedi" placeholder="目的地" clearable  :readonly="ro.mudedi"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="目的地" prop="mudedi">
					<el-input v-model="ruleForm.mudedi" placeholder="目的地" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="出发时间" prop="chufashijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.chufashijian" label="出发时间" prop="chufashijian">
					<el-input v-model="ruleForm.chufashijian" placeholder="出发时间" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="预计天数" prop="yujitianshu">
					<el-input v-model="ruleForm.yujitianshu" placeholder="预计天数" clearable  :readonly="ro.yujitianshu"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="预计天数" prop="yujitianshu">
					<el-input v-model="ruleForm.yujitianshu" placeholder="预计天数" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="旅游路线" prop="lvyouluxian">
					<el-input
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.lvyouluxian" label="旅游路线" prop="lvyouluxian">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}'>{{ruleForm.lvyouluxian}}</span>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="规划内容" prop="guihuaneirong">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.guihuaneirong" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.guihuaneirong" label="规划内容" prop="guihuaneirong">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.guihuaneirong"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='yujitianshu'){
							this.ruleForm.yujitianshu = obj[o];
							this.ro.yujitianshu = true;
							continue;
						}
						if(o=='lvyouluxian'){
						...
						if(o=='guihuaneirong'){
							this.ruleForm.guihuaneirong = obj[o];
							this.ro.guihuaneirong = true;
							continue;
						}
						if(o=='zhanghao'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.guihuaneirong = this.ruleForm.guihuaneirong.replace(reg,'../../../lvyyoureserve/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `lvyouguihua/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.lvyouguihuaCrossAddOrUpdateFlag = false;
						 this.parent.search();
