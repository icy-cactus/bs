	...
	<div class="main-content">
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{ "width": "10%", "margin": "0 0 20px 10px" }' :inline="true"
				...
				<el-row :style='{ "flexDirection": "column", "display": "flex" }'>
					...
					<div :style='{ "margin": "0 15px 0 0", "display": "inline-block" }' class="select">
						...
						<el-select @change="sfshChange" clearable v-model="searchForm.sfsh" placeholder="是否通过">
							<el-option v-for="(item, index) in sfshOptions" v-bind:key="index" :label="item"
				...
				<el-row :style='{ "margin": "20px 0", "flexDirection": "column", "display": "flex" }'>
					<el-button
						...
						v-if="isAuth('ticketreserve', '新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button
						:style='{ "border": "0", "cursor": "pointer", "padding": "0", "boxShadow": "0px 2px 6px 0px rgba(0,0,0,0.3020)", "margin": "0 0 10px", "outline": "none", "color": "#fff", "borderRadius": "30px", "background": "#BAD399", "width": "100%", "fontSize": "14px", "height": "40px" }'
						v-if="isAuth('ticketreserve', '删除')" :disabled="dataListSelections.length <= 0" type="danger"
			...
			<el-table class="tables" :stripe='false'
				...
				v-if="isAuth('ticketreserve', '查看')" :data="dataList" v-loading="dataListLoading"
				@selection-change="selectionChangeHandler">
				<el-table-column :resizable='true' type="selection" align="center" width="50"></el-table-column>
				<el-table-column :resizable='true' :sortable='false' label="索引" type="index" width="50" />
				<el-table-column :resizable='true' :sortable='false' prop="scenicname" label="景点名称">
					<template slot-scope="scope">
						{{ scope.row.scenicname }}
					</template>
				</el-table-column>
				<el-table-column :resizable='true' :sortable='false' prop="address" label="景点地址">
					<template slot-scope="scope">
						{{ scope.row.address }}
					</template>
				</el-table-column>
				<el-table-column :resizable='true' :sortable='false' prop="price" label="门票价格">
					<template slot-scope="scope">
						{{ scope.row.price }}
					</template>
				</el-table-column>
				<el-table-column :resizable='true' :sortable='false' prop="sceniclevel" label="景点等级">
					<template slot-scope="scope">
				...
				<el-table-column :resizable='true' :sortable='false' prop="remark" label="备注">
					<template slot-scope="scope">
						{{ scope.row.remark }}
					</template>
				</el-table-column>
				...
				<el-table-column :resizable='true' :sortable='false' prop="ispay" label="是否支付">
					<template slot-scope="scope">
						...
						<el-button v-if="scope.row.ispay != '已支付' && isAuth('ticketreserve', '支付')" type="text" size="small"
				...
				<el-table-column width="300" label="操作">
					<template slot-scope="scope">
						<el-button
							...
							v-if="isAuth('ticketreserve', '查看')" type="success" size="mini"
							@click="addOrUpdateHandler(scope.row.id, 'info')">详情</el-button>
						<el-button
							:style='{ "border": "0", "cursor": "pointer", "padding": "0 24px", "margin": "0 10px 4px 0", "outline": "none", "color": "#000", "borderRadius": "30px", "background": "#E4E4E4", "width": "auto", "fontSize": "14px", "height": "32px" }'
							v-if="isAuth('ticketreserve', '修改')" type="primary" size="mini"
						...
						<el-button
							:style='{ "border": "0", "cursor": "pointer", "padding": "0 24px", "margin": "0 10px 4px 0", "outline": "none", "color": "#000", "borderRadius": "30px", "background": "#E4E4E4", "width": "auto", "fontSize": "14px", "height": "32px" }'
							v-if="isAuth('ticketreserve', '删除')" type="danger" size="mini"
...
export default {
	...
	methods: {
		...
		payHandler(row) {
			this.$storage.set('paytable', 'ticketreserve');
		...
		init() {
			this.sfshOptions = "待审核,已取消,已通过,已使用,已评价".split(',');
		},
		search() {
			this.pageIndex = 1;
			this.getDataList();
