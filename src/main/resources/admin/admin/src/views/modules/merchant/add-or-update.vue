		...
		<el-form
			...
			<template>
				<el-form-item :style='{ "width": "100%", "margin": "0 0 20px 0" }' class="input" v-if="type != 'info'"
					label="商家账号" prop="merchantusername">
					<el-input v-model="ruleForm.merchantusername" placeholder="商家账号" clearable
						:readonly="ro.merchantusername"></el-input>
				</el-form-item>
				<el-form-item :style='{ "width": "100%", "margin": "0 0 20px 0" }' v-else class="input" label="商家账号"
				...
				<el-form-item :style='{ "width": "100%", "margin": "0 0 20px 0" }' class="input" v-if="type != 'info'"
					...
					<el-input v-model="ruleForm.merchantaddress" placeholder="商家地址" clearable
						:readonly="ro.merchantaddress"></el-input>
				</el-form-item>
				<el-form-item :style='{ "width": "100%", "margin": "0 0 20px 0" }' v-else class="input" label="商家地址"
					prop="merchantaddress">
					<el-input v-model="ruleForm.merchantaddress" placeholder="商家地址" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{ "width": "100%", "margin": "0 0 20px 0" }' class="input" v-if="type != 'info'"
					label="邮箱" prop="email">
					<el-input v-model="ruleForm.email" placeholder="邮箱" clearable :readonly="ro.email"></el-input>
				</el-form-item>
				<el-form-item :style='{ "width": "100%", "margin": "0 0 20px 0" }' v-else class="input" label="邮箱"
					prop="email">
					<el-input v-model="ruleForm.email" placeholder="邮箱" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{ "width": "100%", "margin": "0 0 20px 0" }' class="input" v-if="type != 'info'"
					label="联系手机" prop="phonenumber">
					<el-input v-model="ruleForm.phonenumber" placeholder="联系手机" clearable
						:readonly="ro.phonenumber"></el-input>
				</el-form-item>
				<el-form-item :style='{ "width": "100%", "margin": "0 0 20px 0" }' v-else class="input" label="联系手机"
					prop="phonenumber">
					<el-input v-model="ruleForm.phonenumber" placeholder="联系手机" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{ "width": "100%", "margin": "0 0 20px 0" }' class="upload"
					v-if="type != 'info' && !ro.merchantlicense" label="营业执照" prop="merchantlicense">
					<file-upload tip="点击上传营业执照" action="file/upload" :limit="3" :multiple="true"
						:fileUrls="ruleForm.merchantlicense ? ruleForm.merchantlicense : ''"
						@change="merchantlicenseUploadChange"></file-upload>
				</el-form-item>
				<el-form-item :style='{ "width": "100%", "margin": "0 0 20px 0" }' class="upload"
					v-else-if="ruleForm.merchantlicense" label="营业执照" prop="merchantlicense">
					<img v-if="ruleForm.merchantlicense.substring(0, 4) == 'http'" class="upload-img"
						style="margin-right:20px;" v-bind:key="index" :src="ruleForm.merchantlicense.split(',')[0]"
						width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index"
						v-for="(item, index) in ruleForm.merchantlicense.split(',')" :src="$base.url + item" width="100"
						height="100">
				</el-form-item>
			</template>
		...
		init(id, type) {
			...
			if (this.type == 'info' || this.type == 'else') {
				...
				for (var o in obj) {
					...
					if (o == 'merchantaddress') {
						this.ruleForm.merchantaddress = obj[o];
						this.ro.merchantaddress = true;
						continue;
					}
					if (o == 'email') {
					...
					if (o == 'merchantlicense') {
						this.ruleForm.merchantlicense = obj[o];
						this.ro.merchantlicense = true;
						continue;
					}
					if (o == 'money') {
						...
						this.$http({
							...
							if (data && data.code === 0) {
								if (data.data.total >= crossoptnum) {
									...
									this.$http({
										url: `merchant/${!this.ruleForm.id ? "save" : "update"}`,
										method: "post",
										data: this.ruleForm
									}).then(({ data }) => {
										if (data && data.code === 0) {
											this.$message({
												...
												onClose: () => {
													...
													this.parent.merchantCrossAddOrUpdateFlag = false;
													this.parent.search();
