    ...
    >
      ...
      >
        ...
        <el-form-item :style="{ margin: '10px' }">
         ...
         <el-select
            v-model="formSearch.hoteltype"
            placeholder="请选择酒店类型"
            clearable
          >
            <el-option
              v-for="item in hoteltypeOptions"
              :key="item.indexName"
      ...
      <div class="list" :style="{ margin: '20px 0 0 0', flex: '1' }">
        ...
        >
          ...
          <!-- 酒店图片区域 -->
          <img
            v-if="item.hotelphoto && item.hotelphoto.substr(0, 4) === 'http'"
            :src="item.hotelphoto"
            class="list-image"
          />
          <img
...
export default {
  ...
  created() {
    ...
    this.roomtypeOptions =
      "单人房,大床房,双人间,三人间,标准套间,豪华套间".split(",");
    this.getFenlei();
    this.getList(1, "全部");
  },
  //方法集合
  methods: {
    ...
    getFenlei() {
      this.$http({
        ...
        if (data && data.code === 0) {
          this.hoteltypeOptions = data.data.list;
        }
      });
      this.$http({
        url: `dictionary/page`,
        ...
        if (data && data.code === 0) {
          this.roomtypeOptions = data.data.list;
        }
      });
    },
    getList(page, fenlei, ref = "") {
      ...
      let searchWhere = {};
      if (this.formSearch.hotelname != "")
        searchWhere.hotelname = "%" + this.formSearch.hotelname + "%";
      if (this.formSearch.hoteltype != "")
        searchWhere.hoteltype = this.formSearch.hoteltype;
      if (this.formSearch.roomtype != "")
        searchWhere.roomtype = this.formSearch.roomtype;
      this.$http
        .get("hotelinfo/list", { params: Object.assign(params, searchWhere) })
        .then((res) => {
          if (res.data.code == 0) {
