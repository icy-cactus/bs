			...
			<div class="attr"
				...
				<el-carousel
					...
					<el-carousel-item :style='{ "borderRadius": "8px", "width": "100%", "height": "100%" }'
						...
						<el-image :style='{ "width": "100%", "objectFit": "cover", "borderRadius": "8px", "height": "100%" }'
				...
				<div class="zancai"
					...
					<div :style='{ "cursor": "pointer", "padding": "10px 10px", "margin": "0 20px 0 0", "alignItems": "center", "borderRadius": "8px", "background": "#bad399", "display": "flex", "width": "auto", "justifyContent": "center" }'
						...
						<span
							:style='{ "color": "#fff", "margin": "0 3px", "fontSize": "14px" }'>赞一下({{ detail.thumbsupnum }})</span>
					</div>
					<div :style='{ "cursor": "pointer", "padding": "10px 10px", "margin": "0 20px 0 0", "alignItems": "center", "borderRadius": "8px", "background": "#bad399", "display": "flex", "width": "auto", "justifyContent": "center" }'
						...
						<span
							:style='{ "color": "#fff", "margin": "0 3px", "fontSize": "14px" }'>踩一下({{ detail.crazilynum }})</span>
					</div>
					<div :style='{ "cursor": "pointer", "padding": "10px 10px", "margin": "0 20px 0 0", "alignItems": "center", "borderRadius": "8px", "background": "#bad399", "display": "flex", "width": "auto", "justifyContent": "center" }'
						...
						<i :style='{ "color": "#fff", "margin": "0 3px", "fontSize": "14px" }'
							class="iconfont icon-dianzan"></i>
						<span
							:style='{ "color": "#fff", "margin": "0 3px", "fontSize": "14px" }'>取消赞({{ detail.thumbsupnum }})</span>
					</div>
					<div :style='{ "cursor": "pointer", "padding": "10px 10px", "margin": "0 20px 0 0", "alignItems": "center", "borderRadius": "8px", "background": "#bad399", "display": "flex", "width": "auto", "justifyContent": "center" }'
						...
						<i :style='{ "color": "#fff", "margin": "0 3px", "fontSize": "14px" }'
							class="iconfont icon-iconfontzan"></i>
						<span
							:style='{ "color": "#fff", "margin": "0 3px", "fontSize": "14px" }'>取消踩({{ detail.crazilynum }})</span>
					</div>
				</div>
				...
				<div class="info"
					...
					<div class="item"
						...
						<div :style='{ "color": "#333", "fontSize": "16px", "fontWeight": "bold" }'>
							{{ detail.hotelname }}<span class="rating-score">★{{ detail.rating ||0.0}}/10.0</span>
						</div>
						...
						<div @click="storeup(-1)" v-show="isStoreup"
							:style='{ "cursor": "pointer", "padding": "10px", "borderRadius": "8px", "background": "#bad399" }'>
							<i v-if="true" :style='{ "color": "#fff", "fontSize": "14px" }' class="el-icon-star-on"></i><span
					...
					<div class="btn" :style='{ "padding": "10px 0 0", "flexWrap": "wrap", "display": "flex" }'>
						<el-button
							...
							v-if="isAuth('hotelinfo', '订酒店')" @click="onAcross('hotelreserve', '', '', '')"
							type="warning">订酒店</el-button>
					</div>
				</div>
...
export default {
	...
	methods: {
		init() {
			...
			this.$http.get(this.tablename + '/detail/' + this.detail.id, {}).then(res => {
				if (res.data.code == 0) {
					...
					this.title = this.detail.hotelname;
					this.detailBanner = this.detail.hotelphoto ? this.detail.hotelphoto.split(",") : [];
					this.$forceUpdate();
				}
			});
		...
		onAcross(acrossTable, crossOptAudit, statusColumnName, tips, statusColumnValue) {
			localStorage.setItem('crossTable', `hotelinfo`);
		...
		getStoreupStatus() {
			if (localStorage.getItem("Token")) {
				this.$http.get('storeup/list', { params: { page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'hotelinfo', userid: localStorage.getItem('userid') } }).then(res => {
		...
		thumbsupOrCrazily(type) {
			...
			this.$http.post('hotelinfo/update', this.detail).then(res => { });
		},
		...
		getThumbsupOrCrazilyStatus() {
			if (localStorage.getItem("Token")) {
				this.$http.get('storeup/list', { params: { page: 1, limit: 1, type: 21, refid: this.detail.id, tablename: 'hotelinfo', userid: localStorage.getItem('userid') } }).then(res => {
					if (res.data.code == 0 && res.data.data.list.length > 0) {
				...
				this.$http.get('storeup/list', { params: { page: 1, limit: 1, type: 22, refid: this.detail.id, tablename: 'hotelinfo', userid: localStorage.getItem('userid') } }).then(res => {
					if (res.data.code == 0 && res.data.data.list.length > 0) {
		...
		getDiscussList(page) {
			this.$http.get('discusshotelinfo/list', { params: { page, limit: this.pageSize, refid: this.detail.id } }).then(res => {
				if (res.data.code == 0) {
			...
			this.$http.get('hotelreserve/list', { params: { page, limit: this.pageSize, refid: this.detail.id, sfsh: '已评价' } }).then(res => {
				if (res.data.code == 0) {
		...
		submitForm(formName) {
			...
			this.$refs[formName].validate((valid) => {
				if (valid) {
					...
					this.$http.post('discusshotelinfo/add', this.form).then(res => {
						if (res.data.code == 0) {
							this.form.content = '';
							this.getDiscussList(1);
							this.$message({
