				...
				<div class="info"
					...
					<div class="item"
						...
						<div :style='{ "color": "#333", "fontSize": "16px", "fontWeight": "bold" }'>
							{{ detail.scenicname }}<span class="rating-score">★{{ detail.rating ||0.0}}/10.0</span>
						</div>
					...
					<div class="btn" :style='{ "padding": "10px 0 0", "flexWrap": "wrap", "display": "flex" }'>
						<el-button
							...
							v-if="isAuth('scenicinfo', '订门票')" @click="onAcross('ticketreserve', '', '', '')"
							type="warning">订门票</el-button>
					</div>
				</div>
				...
				<el-tab-pane label="地图" name="first">
					...
					<div style="width:75%;height:75%;margin: auto">
						<iframe MARGINWIDTH=0 MARGINHEIGHT=0 HSPACE=0 VSPACE=0 FRAMEBORDER=0 SCROLLING=no
							src="https://map.baidu.com/" height="600px" width="100%"></iframe>
					</div>

				</el-tab-pane>


				<el-tab-pane label="景点介绍" name="second">
...
export default {
	...
	methods: {
		...
		init() {
			...
			this.$http.get(this.tablename + '/detail/' + this.detail.id, {}).then(res => {
				if (res.data.code == 0) {
					...
					this.title = this.detail.scenicname;
					this.detailBanner = this.detail.scenicphoto ? this.detail.scenicphoto.split(",") : [];
					this.$forceUpdate();
				}
			});
		...
		onAcross(acrossTable, crossOptAudit, statusColumnName, tips, statusColumnValue) {
			localStorage.setItem('crossTable', `scenicinfo`);
		...
		getStoreupStatus() {
			if (localStorage.getItem("Token")) {
				this.$http.get('storeup/list', { params: { page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'scenicinfo', userid: localStorage.getItem('userid') } }).then(res => {
		...
		thumbsupOrCrazily(type) {
			...
			this.$http.post('scenicinfo/update', this.detail).then(res => { });
		},
		...
		getThumbsupOrCrazilyStatus() {
			if (localStorage.getItem("Token")) {
				this.$http.get('storeup/list', { params: { page: 1, limit: 1, type: 21, refid: this.detail.id, tablename: 'scenicinfo', userid: localStorage.getItem('userid') } }).then(res => {
					if (res.data.code == 0 && res.data.data.list.length > 0) {
				...
				this.$http.get('storeup/list', { params: { page: 1, limit: 1, type: 22, refid: this.detail.id, tablename: 'scenicinfo', userid: localStorage.getItem('userid') } }).then(res => {
					if (res.data.code == 0 && res.data.data.list.length > 0) {
		...
		getDiscussList(page) {
			this.$http.get('discussscenicinfo/list', { params: { page, limit: this.pageSize, refid: this.detail.id } }).then(res => {
				if (res.data.code == 0) {
			...
			this.$http.get('ticketreserve/list', { params: { page, limit: this.pageSize, refid: this.detail.id, sfsh: '已评价' } }).then(res => {
				if (res.data.code == 0) {
		...
		submitForm(formName) {
			...
			this.$refs[formName].validate((valid) => {
				if (valid) {
					...
					this.$http.post('discussscenicinfo/add', this.form).then(res => {
						if (res.data.code == 0) {
							this.form.content = '';
							this.getDiscussList(1);
							this.$message({
