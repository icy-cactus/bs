...
<div :style='{"border":"2px solid #bad399","padding":"20px","margin":"20px auto 0","borderRadius":"8px","flexWrap":"wrap","background":"#fff","width":"1200px","position":"relative"}'>
    <el-form
      ...
      label-width="200px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="旅行地点" prop="address">
            <el-input v-model="ruleForm.address"
                placeholder="旅行地点" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}'  label="类型" prop="traveltype">
            <el-select v-model="ruleForm.traveltype" placeholder="请选择类型"  >
              <el-option
                  v-for="(item,index) in traveltypeOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="旅行天数" prop="duration">
            <el-input v-model="ruleForm.duration"
                placeholder="旅行天数" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="旅行图片" v-if="type!='cross' || (type=='cross' && !ro.travelphoto)" prop="travelphoto">
            <file-upload
            tip="点击上传旅行图片"
            action="file/upload"
            :limit="3"
            ...
            @change="travelphotoUploadChange"
            ></file-upload>
          </el-form-item>
            <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' class="upload" v-else label="旅行图片" prop="travelphoto">
                <img v-if="ruleForm.travelphoto.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.travelphoto.split(',')[0]" width="100" height="100">
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="item" v-for="(item,index) in ruleForm.travelphoto.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="旅行内容" prop="content">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='travelphoto'){
              this.ruleForm.travelphoto = obj[o].split(",")[0];
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('travelguide/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('travelguide/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('travelguide/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
      ...
      travelphotoUploadChange(fileUrls) {
          this.ruleForm.travelphoto = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
