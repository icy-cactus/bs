    ...
    <div class="list-preview" :style="{
      ...
      <el-form :inline="true" :model="formSearch" class="list-form-pv" :style="{
        ...
        <el-form-item :style="{ margin: '10px' }">
          ...
          <el-input v-model="formSearch.address" placeholder="旅游地点" clearable></el-input>
        </el-form-item>
        <el-form-item :style="{ margin: '10px' }">
          <div class="lable" v-if="true" :style="{
            width: 'auto',
          ...
          <el-select v-model="formSearch.traveltype" placeholder="请选择攻略类型" :clearable="true">
            <el-option v-for="(item, index) in traveltypeOptions" :key="index" :label="item.indexName"
        ...
        <el-button v-if="isAuth('travelguide', '新增')" :style="{
          cursor: 'pointer',
          border: '0',
          padding: '0px 15px',
          margin: '10px',
          ...
          lineHeight: '42px',
          height: '42px',
        }" type="primary" @click="add('/index/travelguideAdd')"><i v-if="true"
      ...
      <div class="list" :style="{ margin: '20px 0 0 0', flex: '1' }">
        <div v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-card">
          ...
          <!-- 路线图片区域 -->
          <img v-if="item.travelphoto && item.travelphoto.substr(0, 4) === 'http'" :src="item.travelphoto"
            class="list-image" />
          <img v-else :src="baseUrl + (item.travelphoto ? item.travelphoto.split(',')[0] : '')
            " class="list-image" />
  ...
  methods: {
    ...
    getFenlei() {
      this.$http({
        ...
        if (data && data.code === 0) {
          this.traveltypeOptions = data.data.list;
        }
      });
    ...
    getList(page, fenlei, ref = "") {
      ...
      if (this.formSearch.traveltype != "")
        searchWhere.traveltype = this.formSearch.traveltype;
      this.$http
        .get("travelguide/list", { params: Object.assign(params, searchWhere) })
        .then((res) => {
          if (res.data.code == 0) {
