			...
			<div class="list-layout-container">
				...
				<div class="table-area">
					<el-table class="tables" :stripe='false'
						...
						<el-table-column :resizable='true' :sortable='false' prop="hotelname" label="酒店名称">
							<template slot-scope="scope">{{ scope.row.hotelname }}</template>
						</el-table-column>
						<el-table-column :resizable='true' :sortable='false' prop="hoteltype" label="酒店类型">
							<template slot-scope="scope">{{ scope.row.hoteltype }}</template>
						</el-table-column>
						<el-table-column :resizable='true' :sortable='false' prop="address" label="酒店地址">
							<template slot-scope="scope">{{ scope.row.address }}</template>
						</el-table-column>
						<el-table-column :resizable='true' :sortable='false' prop="roomtype" label="房间类型">
							<template slot-scope="scope">{{ scope.row.roomtype }}</template>
						</el-table-column>
						<el-table-column :resizable='true' :sortable='false' prop="roomno" label="房间号">
							<template slot-scope="scope">{{ scope.row.roomno }}</template>
						</el-table-column>
						<el-table-column :resizable='true' :sortable='false' prop="price" label="预约价格">
						...
						<el-table-column :resizable='true' :sortable='false' prop="reserveremark" label="预订说明">
							<template slot-scope="scope">{{ scope.row.reserveremark }}</template>
						</el-table-column>
						...
						<el-table-column width="300" label="操作">
							<template slot-scope="scope">
								...
								<el-button
									:style='{ "border": "0", "cursor": "pointer", "padding": "0 24px", "margin": "0 10px 4px 0", "outline": "none", "color": "#000", "borderRadius": "30px", "background": "#E4E4E4", "width": "auto", "fontSize": "14px", "height": "32px" }'
									v-if="isAuth('hotelreserve', '修改')" type="primary" size="mini"
								...
								<el-button
									:style='{ "border": "0", "cursor": "pointer", "padding": "0 24px", "margin": "0 10px 4px 0", "outline": "none", "color": "#000", "borderRadius": "30px", "background": "#E4E4E4", "width": "auto", "fontSize": "14px", "height": "32px" }'
									v-if="isAuth('hotelreserve', '删除')" type="danger" size="mini"
					...
					<el-form class="center-form-pv" :style='{ "width": "100%", "margin": "0 0 20px 0" }' :inline="true"
						...
						<el-row :style='{ "flexDirection": "column", "display": "flex" }'>
							<div :style='{ "margin": "0", "flexDirection": "column", "display": "flex" }'>
								<label
									...
									class="item-label">酒店名称</label>
								<el-input v-model="searchForm.hotelname" placeholder="酒店名称" clearable></el-input>
							</div>
							<div :style='{ "margin": "0", "flexDirection": "column", "display": "flex" }'>
								<label
									:style='{ "margin": "0 10px 0 0", "color": "#000", "textAlign": "center", "display": "inline-block", "width": "100%", "lineHeight": "40px", "fontSize": "14px", "fontWeight": "500", "height": "40px" }'
									class="item-label">房间类型</label>
								<el-input v-model="searchForm.roomtype" placeholder="房间类型" clearable></el-input>
							</div>
							<div :style='{ "margin": "0", "flexDirection": "column", "display": "flex" }'>
								<label
									:style='{ "margin": "0 10px 0 0", "color": "#000", "textAlign": "center", "display": "inline-block", "width": "100%", "lineHeight": "40px", "fontSize": "14px", "fontWeight": "500", "height": "40px" }'
							...
							<div :style='{ "margin": "0", "flexDirection": "column", "display": "flex" }'>
								<label
									:style='{ "margin": "0 10px 0 0", "color": "#000", "textAlign": "center", "display": "inline-block", "width": "100%", "lineHeight": "40px", "fontSize": "14px", "fontWeight": "500", "height": "40px" }'
									class="item-label">手机</label>
								<el-input v-model="searchForm.telephone" placeholder="手机" clearable></el-input>
							</div>
							<el-button
						...
						<el-row :style='{ "margin": "20px 0", "flexDirection": "column", "display": "flex" }'>
							<el-button
								...
								v-if="isAuth('hotelreserve', '新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
							<el-button
								:style='{ "border": "0", "cursor": "pointer", "padding": "0", "boxShadow": "0px 2px 6px 0px rgba(0,0,0,0.3020)", "margin": "0 0 10px", "outline": "none", "color": "#fff", "borderRadius": "30px", "background": "#bad399", "width": "100%", "fontSize": "14px", "height": "40px" }'
								v-if="isAuth('hotelreserve', '删除')" :disabled="dataListSelections.length <= 0" type="danger"
	...
	methods: {
		...
		shHandler() {
			this.$confirm(`确定操作?`, "提示", {
				...
				this.$http.post("hotelreserve/update",this.shForm).then(({ data }) => {
					if (data && data.code === 0) {
						this.$message({
							message: "操作成功",
							type: "success",
		...
		payHandler(row) {
			this.$storage.set('paytable', 'hotelreserve');
		...
		getDataList() {
			...
			if (this.searchForm.hotelname != '' && this.searchForm.hotelname != undefined) {
				params['hotelname'] = '%' + this.searchForm.hotelname + '%'
			}
			if (this.searchForm.roomtype != '' && this.searchForm.roomtype != undefined) {
				params['roomtype'] = '%' + this.searchForm.roomtype + '%'
			}
			if (this.searchForm.name != '' && this.searchForm.name != undefined) {
				params['name'] = '%' + this.searchForm.name + '%'
			}
			if (this.searchForm.telephone != '' && this.searchForm.telephone != undefined) {
...
.table-area {
  flex: 1; // 占满剩余宽度
  margin: 0 40px; // 表格左右空余（可根据需求调整数值）
}

// 筛选栏区域：右侧固定宽度，避免过宽或过窄
.filter-area {
  width: 240px; // 筛选栏固定宽度（可根据内容调整）
}
