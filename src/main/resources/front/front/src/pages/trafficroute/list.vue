<template>
	<div>
		<div :style='{ "width": "80%", "padding": "10px", "margin": "12px auto 0", "borderRadius": "16px", "background": "#BAD399" }'
		...
		<div class="list-preview"
			:style='{ "width": "1280px", "margin": "10px auto", "position": "relative", "flexWrap": "wrap", "background": "none", "display": "flex" }'>


			<el-form :inline="true" :model="formSearch" class="list-form-pv"
				:style='{ "border": "2px solid #bad399", "padding": "10px 0", "margin": "10px 0 0 0", "borderRadius": "8px", "flexWrap": "wrap", "background": "#FAFFE6", "display": "flex", "width": "100%", "height": "auto" }'>
				<el-form-item :style='{ "margin": "10px" }'>
					<div class="lable" v-if="true"
						:style='{ "width": "auto", "padding": "0 10px", "lineHeight": "42px", "color": "#29292a", "display": "inline-block" }'>
						路线名称</div>
					<el-input v-model="formSearch.trafficname" placeholder="路线名称" clearable></el-input>
				</el-form-item>
				<el-form-item :style='{ "margin": "10px" }'>
					<div class="lable" v-if="true"
						:style='{ "width": "auto", "padding": "0 10px", "lineHeight": "42px", "color": "#29292a", "display": "inline-block" }'>
						路线类型</div>
					<el-select v-model="formSearch.traffictype" placeholder="请选择路线类型" clearable>
						<el-option v-for="item in traffictypeOptions" :key="item.indexName" :label="item.indexName"
				...
				<el-form-item :style='{ "margin": "10px" }'>
					<div class="lable" v-if="true"
						:style='{ "width": "auto", "padding": "0 10px", "lineHeight": "42px", "color": "#29292a", "display": "inline-block" }'>
						始发地</div>
					<el-input v-model="formSearch.origin" placeholder="始发地" clearable></el-input>
				</el-form-item>
				<el-form-item :style='{ "margin": "10px" }'>
					<div class="lable" v-if="true"
						:style='{ "width": "auto", "padding": "0 10px", "lineHeight": "42px", "color": "#29292a", "display": "inline-block" }'>
						终点地</div>
					<el-input v-model="formSearch.destination" placeholder="终点地" clearable></el-input>
				</el-form-item>
				<el-button v-if="true"
					:style='{ "cursor": "pointer", "border": "0", "padding": "0px 15px", "margin": "10px", "outline": "none", "color": "#fff", "borderRadius": "10px", "background": "#bad399", "width": "auto", "fontSize": "14px", "lineHeight": "42px", "height": "42px" }'
				...
				<el-button v-if="isAuth('trafficroute', '新增')"
					:style='{ "cursor": "pointer", "border": "0", "padding": "0px 15px", "margin": "10px", "outline": "none", "color": "#fff", "borderRadius": "10px", "background": "#bad399", "width": "auto", "fontSize": "14px", "lineHeight": "42px", "height": "42px" }'
					type="primary" @click="add('/index/trafficrouteAdd')"><i v-if="true"
						:style='{ "color": "#fff", "margin": "0 10px 0 0", "fontSize": "14px" }'
			...
			<div class="list" :style='{ "margin": "20px 0 0 0", "flex": "1" }'>
				<div v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-card">
					...
					<!-- 路线图片区域 -->
					<img v-if="item.trafficphoto && item.trafficphoto.substr(0, 4) === 'http'" :src="item.trafficphoto"
						class="list-image" />
					<img v-else :src="baseUrl + (item.trafficphoto ? item.trafficphoto.split(',')[0] : '')
						" class="list-image" />
			...
			<el-pagination background class="pagination" :pager-count="7" :page-size="pageSize" :page-sizes="pageSizes"
				...
				:style='{ "width": "1280px", "margin": "20px auto", "whiteSpace": "nowrap", "color": "#333" }'
	...
	methods: {
		...
		getFenlei() {
			this.$http({
				...
				if (data && data.code === 0) {
					this.traffictypeOptions = data.data.list;
				}
			});
		},
		getList(page, fenlei, ref = '') {
			...
			let searchWhere = {};
			if (this.formSearch.trafficname != '') searchWhere.trafficname = '%' + this.formSearch.trafficname + '%';
			if (this.formSearch.traffictype != '') searchWhere.traffictype = this.formSearch.traffictype;
			if (this.formSearch.origin != '') searchWhere.origin = '%' + this.formSearch.origin + '%';
			if (this.formSearch.destination != '') searchWhere.destination = '%' + this.formSearch.destination + '%';
			this.$http.get('trafficroute/list', { params: Object.assign(params, searchWhere) }).then(res => {
				if (res.data.code == 0) {
		...
		toDetail(item) {
			this.$router.push({ path: '/index/trafficrouteDetail', query: { detailObj: JSON.stringify(item) } });
		},
	}
}
</script>
