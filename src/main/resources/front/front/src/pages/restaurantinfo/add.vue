...
<div :style='{"border":"2px solid #bad399","padding":"20px","margin":"20px auto 0","borderRadius":"8px","flexWrap":"wrap","background":"#fff","width":"1200px","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="人均消费" prop="avgprice">
            <el-input v-model="ruleForm.avgprice" 
                placeholder="人均消费" clearable ></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="餐厅图片" v-if="type!='cross' || (type=='cross' && !ro.restaurantphoto)" prop="restaurantphoto">
            <file-upload
            tip="点击上传餐厅图片"
            action="file/upload"
            :limit="3"
            ...
            @change="restaurantphotoUploadChange"
            ></file-upload>
          </el-form-item>
            <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' class="upload" v-else label="餐厅图片" prop="restaurantphoto">
                <img v-if="ruleForm.restaurantphoto.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.restaurantphoto.split(',')[0]" width="100" height="100">
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.restaurantphoto.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="餐厅介绍" prop="introduce">
            <editor 
...
<script>
  export default {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='restaurantphoto'){
              this.ruleForm.restaurantphoto = obj[o].split(",")[0];
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('restaurantinfo/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('restaurantinfo/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('restaurantinfo/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
      ...
      restaurantphotoUploadChange(fileUrls) {
          this.ruleForm.restaurantphoto = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
