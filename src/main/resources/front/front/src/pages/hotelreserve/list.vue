...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"2px solid #bad399","padding":"10px 0","margin":"10px 0 0 0","borderRadius":"8px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#29292a","display":"inline-block"}'>酒店名称</div>
        <el-input v-model="formSearch.hotelname" placeholder="酒店名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#29292a","display":"inline-block"}'>房间类型</div>
        <el-input v-model="formSearch.roomtype" placeholder="房间类型" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"10px"}'>
      ...
      <el-form-item :style='{"margin":"10px"}'>
        ...
        <el-input v-model="formSearch.telephone" placeholder="手机" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('hotelreserve','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"10px","outline":"none","color":"#fff","borderRadius":"10px","background":"#bad399","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/hotelreserveAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.hotelname != '') searchWhere.hotelname = '%' + this.formSearch.hotelname + '%';
        if (this.formSearch.roomtype != '') searchWhere.roomtype = '%' + this.formSearch.roomtype + '%';
        if (this.formSearch.name != '') searchWhere.name = '%' + this.formSearch.name + '%';
        if (this.formSearch.telephone != '') searchWhere.telephone = '%' + this.formSearch.telephone + '%';
        this.$http.get('hotelreserve/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/hotelreserveDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
